...
#define TOMATO_SYSCALL_OS_PULL_EVENT				0x02
...
#define TOMATO_SYSCALL_TERM_GET_CURSOR_Y			0x19
...
inline static uint32_t tomato_syscall_4p(uint32_t id, uint32_t p1, uint32_t p2, uint32_t p3, uint32_t p4) {
	uint32_t ret;
	asm volatile("int $0x80" : "=a"(ret) : "a"(id), "b"(p1), "c"(p2), "d"(p3), "S"(p4));
	return ret;
}
